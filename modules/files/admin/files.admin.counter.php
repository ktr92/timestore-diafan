 ...
 * Files_admin_counter
 */
class Files_admin_counter extends Frame_admin
{
	/**
	...
	public function prepare_config()
	{
		...
		if ($this->diafan->configmodules("cat", "files"))
		{
			$this->diafan->categories = DB::query_fetch_all(
				"SELECT id, [name], parent_id FROM {files_category} WHERE trash='0'"
	...
	public function show()
	{
		...
		{
			$inner = " INNER JOIN {files_category_rel} AS r ON r.element_id=s.id";
			$where = " AND r.cat_id=".$this->diafan->filter($_GET, "integer", "filter_cat_id");
		}

		$this->diafan->_paginator->navlink = 'files/counter/';
		$this->diafan->_paginator->get_nav = $this->diafan->get_nav;
		$this->diafan->_paginator->nen     = DB::query_result("SELECT COUNT(DISTINCT s.id) FROM {files} AS s".$inner." WHERE s.trash='0'".$where);
		$result["links"] = $this->diafan->_paginator->get();
		
		//забираем все файлы, удовлетворяющие фильтру
		$result["rows"] = DB::query_range_fetch_all("SELECT s.id, s.[name], s.[act], cn.count_view FROM {files} AS s
		LEFT JOIN {files_counter} AS cn ON cn.element_id=s.id
		".$inner."
	...
	{
		if(! $this->diafan->configmodules("counter", "files"))
		{
			echo '<div class="error">'.$this->diafan->_('Подключите счетчик просмотров в настройках модуля.').'</div>';
		}
		if ($this->diafan->configmodules("cat", "files"))
		{
			$cats_filter = DB::query_fetch_key_array(
				"SELECT id, [name], parent_id FROM {files_category} WHERE trash='0'"
				." ORDER BY sort ASC, id ASC", "parent_id"
			);
			if($cats_filter)
			{
				echo '<div class="block">'.$this->diafan->_('Категория').': <select rel="'.$this->diafan->get_admin_url('cat', 'site').'" class="redirect" name="cat">'
				. '<option value="">'.$this->diafan->_('Все').'</option>';
				echo $this->diafan->get_options($cats_filter, $cats_filter[0], array($this->diafan->_route->cat))
				. '</select></div>';
			}
		}
		...
		foreach ($result["rows"] as $row)
		{
			...
			<div class="item__in">
			<div><a href="'.BASE_PATH_HREF.'files/edit'.$row["id"].'/">'.($row["name"] ? $row["name"] : $row["id"]).'</a></div>
