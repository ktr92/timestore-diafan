...
class Files_model extends Model
{
	...
	public function list_category()
	{
		...
		if (! empty($this->result["previous"]["text"]))
		{
			$this->result["previous"]["text"] =
				$this->diafan->_useradmin->get($this->result["previous"]["text"], 'name', $this->result["previous"]["id"], 'files_category', _LANG);
		}
		if (! empty($this->result["next"]["text"]))
		{
			$this->result["next"]["text"] =
				$this->diafan->_useradmin->get($this->result["next"]["text"], 'name', $this->result["next"]["id"], 'files_category', _LANG);
		}
	...
	private function list_category_previous_next($sort, $parent_id)
	{
		...
		if ($previous)
		{
			...
			$this->result["previous"]["link"] = $this->diafan->_route->link($this->diafan->_site->id, $previous["id"], "files", 'cat');
		}
		...
		if ($next)
		{
			...
			$this->result["next"]["link"] = $this->diafan->_route->link($this->diafan->_site->id, $next["id"], "files", 'cat');
		}
	}
	...
	public function id()
	{
		...
		{
			$this->result["previous"]["text"] =
				$this->diafan->_useradmin->get($this->result["previous"]["text"], 'name', $this->result["previous"]["id"], 'files', _LANG);
		}
		if (! empty($this->result["next"]["text"]))
		{
			$this->result["next"]["text"] =
				$this->diafan->_useradmin->get($this->result["next"]["text"], 'name', $this->result["next"]["id"], 'files', _LANG);
		}
		foreach ($this->result["breadcrumb"] as $k => &$b)
		{
			if ($k == 0)
	...
	private function id_query($time)
	{
		...
		$row = DB::query_fetch_array("SELECT id, [name], [anons], [anons_plus], [text], timeedit, cat_id, [keywords], [descr], [canonical], sort, [title_meta], access, theme, view, [act], noindex".$fields." FROM {files}"
		." WHERE id=%d AND trash='0' AND site_id=%d"
	...
	private function id_previous_next($sort, $time)
	{
		...
		$this->diafan->_site->id, $sort, $sort, $this->diafan->_route->show, $time, $time
		);
		$next = DB::query_fetch_array(
		"SELECT e.[name], e.id FROM {files} AS e"
		. ($this->diafan->configmodules('where_access_element') ? " LEFT JOIN {access} AS a ON a.element_id=e.id AND a.module_name='files' AND a.element_type='element'" : "")
		...
		if ($previous)
		{
			$this->result["previous"]["text"] = $previous["name"];
			$this->result["previous"]["id"] = $previous["id"];
			$this->result["previous"]["link"] = $this->diafan->_route->link($this->diafan->_site->id, $previous["id"], "files");
		}
		if ($next)
		{
			$this->result["next"]["text"] = $next["name"];
			$this->result["next"]["id"] = $next["id"];
			$this->result["next"]["link"] = $this->diafan->_route->link($this->diafan->_site->id, $next["id"], "files");
		}
	}
