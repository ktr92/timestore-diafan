...
class Wishlist_inc extends Diafan
{
	...
	public function set($value = array(), $id = 0, $param = false, $additional_cost = false, $name_info = '')
	{
		...
		if($name_info == "count")
		{
			...
			if($this->wishlist[$id][$param][$additional_cost]["is_file"] && $value > 1)
			{
				$value = 1;
			}
		}
		...
		foreach ($this->wishlist as $id => $rows)
		{
			foreach ($rows as $param => $rs)
			{
				...
				foreach ($rs as $additional_cost => $row)
				{
					...
					{
						DB::query("INSERT INTO {shop_wishlist} (good_id, created, count, param, additional_cost, is_file, ".($this->diafan->_users->id ? "user_id" : "session_id").") VALUES (%d, %d, %f, '%s', '%s', '%d', ".($this->diafan->_users->id ? $this->diafan->_users->id : "'".$this->diafan->_session->id."'").")", $id, time(), $row["count"], $param, $additional_cost, $row["is_file"]);
					}
				}
			}
		}
