	...
	private function check_file($module)
	{
		...
		if(REPAIR_ADMINSITE && $class->admin)
		{
			foreach ($class->admin as $row)
			{
				...
				{
					$row["id"] = DB::query("INSERT INTO {admin} (group_id, name, rewrite, act, sort, docs) VALUES ('%s', '%s', '%s', '%s', %d, '%s')", $row['group_id'], $row['name'], $row['rewrite'], $row['act'], $row['sort'], $row["docs"]);
					if (REPAIR_LOG)
					{
				...
				else
				{
					...
					if(REPAIR_ADMINSITE == 2 && ($v["group_id"] != $row["group_id"] || $v["name"] != $row["name"] || $v["docs"] != $row["docs"]))
					{
						DB::query("UPDATE {admin} SET group_id='%s', name='%s', docs='%s' WHERE id=%d", $row['group_id'], $row['name'], $row["docs"], $v["id"]);

						DB::query("UPDATE {admin} SET group_id='%s', docs='%s' WHERE parent_id=%d", $row['group_id'], $row["docs"], $v["id"]);
				...
				if(! empty($row["children"]))
				{
					foreach ($row["children"] as $i => $ch)
					{
						...
						{
							$ch["id"] = DB::query("INSERT INTO {admin} (group_id, name, rewrite, act, sort, docs, parent_id) VALUES ('%s', '%s', '%s', '%s', %d, '%s', %d)", $row['group_id'], $ch['name'], $ch['rewrite'], (! empty($ch['act']) ? 1 : 0), $ch['sort'], $row["docs"], $row["id"]);
