	...
	public function act($table, $element_ids, $act)
	{
		...
		if ($table == "users" && $act)
		{
			...
			foreach ($rows as $row)
			{
				...
				{
					$login = $this->diafan->filter($row["mail"]);
				}
				else
				{
					$login = $this->diafan->filter($row["name"]);
				}
				...
				{
					send_mail(
						$row["mail"], $subject, $message, $this->diafan->configmodules("emailconf", "users") ? $this->diafan->configmodules("email", "users") : ''
					);
				}
			}
		}
