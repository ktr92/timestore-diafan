...
class Payment_admin_history extends Frame_admin
{
	...
	public function list_variable_element_id($row, $var)
	{
		...
		switch($row["module_name"])
		{
			...
			case 'cart':
				$text .= '<a href="'.BASE_PATH_HREF.'shop/order/edit'.$row["element_id"].'/">'.$this->diafan->_('Заказ №%s', $row["element_id"]).'</a>';
				break;
		}
		$text .= '</div>';
		return $text;
	...
	public function edit_variable_element_id()
	{
		...
		switch($this->diafan->values("module_name"))
		{
			...
			case 'cart':
				echo '<a href="'.BASE_PATH_HREF.'shop/order/edit'.$this->diafan->value.'/">'.$this->diafan->_('Заказ №%s', $this->diafan->value).'</a>';
				break;
		}
		echo '
			'.$this->diafan->help().'
