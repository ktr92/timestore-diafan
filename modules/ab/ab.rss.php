...
$site_ids = $this->diafan->_route->id_module('ab');
if(empty($site_ids))
{
...
$rows = DB::query_fetch_all("SELECT e.id, e.created, e.[name], e.[anons], e.site_id FROM {ab} AS e"
...
foreach ($rows as $row)
{
	$link = $this->diafan->_route->link($row["site_id"], $row["id"], "ab");
	if(! $link)
	{
		continue;
	}
	...
	<item>
		...
		.($this->diafan->configmodules("comments", "ab", $row["site_id"]) ? "
...
<rss version="2.0">
	<channel>
		<title>'.$this->diafan->_('Объявления', false).'</title>
		<link>'.BASE_PATH_HREF.'</link>
		<description>'.$this->diafan->_('Последние объявления', false).' '.BASE_URL.'.</description>
