 ...
 * Ab_admin_counter
 */
class Ab_admin_counter extends Frame_admin
{
	/**
	...
	public function prepare_config()
	{
		...
		if ($this->diafan->configmodules("cat", "ab"))
		{
			$this->diafan->categories = DB::query_fetch_all(
				"SELECT id, [name], parent_id FROM {ab_category} WHERE trash='0'"
	...
	public function show()
	{
		...
		{
			$inner = " INNER JOIN {ab_category_rel} AS r ON r.element_id=s.id";
			$where = " AND r.cat_id=".$this->diafan->filter($_GET, "integer", "filter_cat_id");
		}

		$this->diafan->_paginator->navlink = 'ab/counter/';
		$this->diafan->_paginator->get_nav = $this->diafan->get_nav;
		$this->diafan->_paginator->nen     = DB::query_result("SELECT COUNT(DISTINCT s.id) FROM {ab} AS s".$inner." WHERE s.trash='0'".$where);
		$result["links"] = $this->diafan->_paginator->get();

		//забираем все объявления, удовлетворяющие фильтру
		$result["rows"] = DB::query_range_fetch_all("SELECT s.id, s.[name], s.[act], cn.count_view FROM {ab} AS s
		LEFT JOIN {ab_counter} AS cn ON cn.element_id=s.id
		".$inner."
	...
	{
		if(! $this->diafan->configmodules("counter", "ab"))
		{
		...
		foreach ($result["rows"] as $row)
		{
			...
			<div class="item__in">
			<div><a href="'.BASE_PATH_HREF.'ab/edit'.$row["id"].'/">'.($row["name"] ? $row["name"] : $row["id"]).'</a></div>
