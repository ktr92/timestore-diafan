 ...
 * Ab_admin_category
 */
class Ab_admin_category extends Frame_admin
{
	/**
	 * @var string таблица в базе данных
	 */
	public $table = 'ab_category';
	...
	public $variables = array (
		'main' => array (
			...
			'paramcat' => array(
				'type' => 'function',
				'name' => 'Характеристики, прикрепленные к категории',
				'help' => 'Список характеристик, применимых к объявлениями текущей категории.',
				'no_save' => true,
			),
			'anons' => array(
				'type' => 'editor',
		...
		'other_rows' => array (
			...
			'view' => array(
				'type' => 'function',
				'name' => 'Шаблон модуля',
				'help' => 'Шаблон вывода контента модуля на странице списка объявлений в категории (веб-мастеру и программисту, не меняйте этот параметр, если не уверены в результате!).',
			),
			'view_rows' => array(
				'type' => 'function',
			...
			'view_element' => array(
				...
				'help' => 'Шаблон вывода контента модуля на странице отдельного объявления, вложенного в текущую категорию (веб-мастеру и программисту, не меняйте этот параметр, если не уверены в результате!).',
			),
			'sort' => array(
				'type' => 'function',
	...
	public function edit_variable_paramcat()
	{
		...
		$rows = DB::query_fetch_all("SELECT p.[name], p.id FROM {ab_param} AS p"
		...
		<div class="unit" id="param">
			...
			foreach ($rows as $row)
			{
				echo '<a href="'.BASE_PATH_HREF.'ab/param/edit'.$row["id"].'/?filter_cat_id='.$this->diafan->id.'">'.$row["name"].'</a><br>';
			}
			echo '<a class="btn btn_small btn_blue" href="'.BASE_PATH_HREF.'ab/param/cat'.$this->diafan->id.'/addnew1/">
