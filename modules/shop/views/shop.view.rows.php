...
{		
	...
	if(! empty($row["anons"]))
	{
		echo '<div class="params-hit">'.$this->htmleditor($row['anons']).'</div>';
	}
...
/*	if(! empty($row["article"]))
	...
	if(! empty($row['hit']))
	{
		echo '<span class="mark-hit">Хит продаж</span>';
	}
	if(! empty($row['action']))
	{
		echo '<span class="mark-hit">Акция</span>';
	}
	if(! empty($row['new']))
	{
		echo '<span class="mark-hit">Новинка</span>';					
	}
