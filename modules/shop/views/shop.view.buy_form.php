...
if ($result["row"]["price_arr"])
{
	foreach ($result["row"]["price_arr"] as $price)
	{
		...
		echo '<div class="js_shop_param_price shop_param_price shop-item-price"'.$param_code.'>';
			echo '<span class="price"><span class="js_shop_price" summ="'.$price["price_no_format"].'" format_price_1="'.$this->diafan->configmodules("format_price_1", "shop").'" format_price_2="'.$this->diafan->configmodules("format_price_2", "shop").'" format_price_3="'.$this->diafan->configmodules("format_price_3", "shop").'"><span class="slide-price">'.$price["price"].'</span></span> &nbsp;<span class="rouble">a</span>';
...
echo '<div class="js_shop_buy shop_buy to-cart">';
	...
	{
		echo '<input type="button" class="button solid btn-slide-orange" value="'.$this->diafan->_('Купить', false).'" action="buy">';
	}
echo '</div>';
