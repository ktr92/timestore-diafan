...
class Comments_admin_param extends Frame_admin
{
	...
	public $variables = array (
		'main' => array (
			...
			'module_name' => array(
				'type' => 'select',
				'name' => 'Модуль',
				'help' => 'Возможность ограничить применением поля комментариями, прикрепленными к выбранному модулю.',
			),
			'type' => array(
				'type' => 'select',
				'name' => 'Тип',
			...
			'show_in_list' => array(
				...
				'help' => 'Выводит значение поля для комментариев на сайте в списке комментариев',
			),
	...
	public function prepare_config()
	{
		...
		$rows = DB::query_fetch_all("SELECT DISTINCT e.name, o.module_name FROM {config} AS o INNER JOIN {admin} AS e ON o.module_name = e.rewrite WHERE o.name = 'comments' AND o.value = '1' AND e.parent_id = '0'");
		foreach ($rows as &$row)
		{
			$select[$row['module_name']] = $row['name'];
		}
