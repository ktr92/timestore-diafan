...
if (! empty($result["params"]))
{
	foreach ($result["params"] as $row)
	{
		...
		switch ($row["type"])
		{
			...
			case 'checkbox':
				echo '<input name="p'.$row["id"].'" id="comment'.$result["parent_id"].'_p'.$row["id"].'" value="1" type="checkbox"><label for="comment'.$result["parent_id"].'_p'.$row["id"].'">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').'</label>';
				break;
			...
			case 'radio':
				...
				{
					echo '<input name="p'.$row["id"].'" type="radio" value="'.$select["id"].'" id="comment'.$result["parent_id"].'_radio_p'.$row["id"].'_'.$select["id"].'"> <label for="comment'.$result["parent_id"].'_radio_p'.$row["id"].'_'.$select["id"].'">'.$select["name"].'</label>';
				}
				break;
...
if($result['use_mail'])
{
	...
	echo '<input name="mail" type="email" value="">';
