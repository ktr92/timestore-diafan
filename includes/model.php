...
abstract class Model extends Diafan
{
	...
	protected function meta_cat($row)
	{
		...
		elseif($this->diafan->_route->page > 1 || $this->diafan->_route->dpage > 1 || $this->diafan->_route->rpage > 1 || $this->diafan->_route->sort)
		{
			$this->result["canonical"] = $this->diafan->_route->current_link(array("page", "dpage", "sort", "rpage"));
		}
		if(! empty($row["noindex"]))
		{
			$this->result["noindex"] = $row["noindex"];
