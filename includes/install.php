...
class Install extends Diafan
{
	...
	public function admin($array = array(), $parent_id = 0, $parent = array())
	{
		...
		{			
			...
			if(empty($parent_id))
			{
				$last_id = DB::query_result("SELECT id FROM {admin} WHERE rewrite='%s' AND parent_id=0", $values['rewrite']);
			}
			if(! $last_id)
			{
				$last_id = DB::query("INSERT INTO {admin} (parent_id, group_id, name, rewrite, act, sort, docs, count_children, `add`, `add_name`) VALUES (%d, %d, '%s', '%s', '%s', %d, '%s', %d, '%d', '%s')", $parent_id, $values['group_id'], $values['name'], $values['rewrite'], $values['act'], $values['sort'], $values["docs"], $values['count_children'], $values["add"], $values["add_name"]);
			}	
	...
	public function uninstall()
	{
		...
		if(! empty($rws))
		{
			$site_ids2 = DB::query_result("SELECT GROUP_CONCAT(element_id SEPARATOR ',') FROM {rewrite} WHERE rewrite IN ('".implode("','", $rws)."') AND module_name='site' AND element_type='element'");
			if($site_ids2)
			{
