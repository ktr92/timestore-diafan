...
class Route extends Diafan
{
	 ...
	 */
	public $variable_names_site = array('cat', 'param', 'show', 'dpage', 'rpage', 'brand', 'year', 'month', 'day', 'step', 'sort', 'add', 'edit', 'page');
	...
	public function current_link($exclude = '', $include = array())
	{
		...
		foreach ($this->variable_names_site as $arg)
		{
			...
			{
				if (! empty($this->$arg) && ($arg != "page" || $this->page != 1) && ($arg != "dpage" || $this->dpage != 1) && ($arg != "rpage" || $this->rpage != 1))
				{
	...
	public function search($rewrite, $arguments_in_url = false)
	{
		...
		if ($row = DB::query_fetch_array("SELECT module_name, element_id, element_type, rewrite FROM {rewrite} WHERE trash='0' AND rewrite='%h' LIMIT 1", utf::to_utf($rewrite)))
		{
			return $row;
		}
		return false;
	 ...
	 * @return void
	 */
	private function check_element_type($element_type, $mode = false)
	{
