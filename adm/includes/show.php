...
class Show_admin extends Diafan
{
	...
	public function list_row($id = 0, $first_level = true)
	{
		...
		if($first_level)
		{
			echo '<ul class="list list_'.($this->diafan->_admin->module).'s'.(strpos($this->diafan->_admin->rewrite, "category") ? '_category' : '').' list'.($this->diafan->variable_list('plus') ? '_pages' : '_catalog '.($this->diafan->_admin->rewrite != 'shop' ? 'do_auto_width' : ''))
		...
		foreach ($this->diafan->rows as $row)
		{
			...
			foreach($this->diafan->variables_list as $name => $var)
			{
				...
				$var["class"] = (! empty($var["class"]) ? $var["class"] : '').(! empty($var['no_important']) ? (! empty($var["class"]) ? ' ' : '').'no_important' : '');
	...
	public function get_heading()
	{
		...
		foreach($this->diafan->variables_list as $row)
		{
			if(! is_array($row))
			{
				$row = array();
			...
			echo '<div class="item__th'.(! empty($row["class_th"]) ? ' '.$row["class_th"] : '').'">'.(! empty($row["name"]) ? $this->diafan->_($row["name"]) : '').'</div>';
		}
		echo '</li>';
	}
	...
	public function list_variable_checkbox($row, $var)
	{
		...
		{
			$text = '<div class="checkbox'.($var["class"] ? ' '.$var["class"] : '').'">';
			if($this->diafan->check_action($row))
			{
				$text .= '<input type="checkbox" name="ids[]" value="'.$row["id"].'">';
			}
			$text .= '</div>';
		}
		return $text;
	...
	public function group_action_panel($show_filter = false)
	{
		...
		{
			echo str_replace('input_menu_cat_ids_', 'second_input_menu_cat_ids_', $this->cache["group_action_panel"]);
			return;
		}
		...
		{
			$html .= '<option value="delete" confirm="'.$this->diafan->_('Вы действительно хотите удалить запись?').'"'.($value == 'delete' ? ' selected' : '').'>'
		...
		{
			$html .= '<option value="element_site"'.($value == 'element_site' ? ' selected' : '').'>'.$this->diafan->_('Переместить в раздел').'</option>';
		}
		if ($this->diafan->config('element') && count($this->diafan->categories))
		{
			$html .= '<option value="element"'.($value == 'element' ? ' selected' : '').'>';
			if($this->diafan->_admin->module == 'photo')
			{
				$html .= $this->diafan->_('Переместить в альбом');
			}
			else
			{
				$html .= $this->diafan->_('Переместить в категорию');
			}
			$html .= '</option>';
			if($this->diafan->config("element_multiple"))
			{
				$html .= '<option value="element_multi"'.($value == 'element_multi' ? ' selected' : '').'>';
				if($this->diafan->_admin->module == 'photo')
				{
					$html .= $this->diafan->_('Дополнительный альбом');
				}
				else
				{
					$html .= $this->diafan->_('Дополнительная категория');
				}
				$html .= '</option>';
			}
			$html .= '<option value="element_del"'.($value == 'element_del' ? ' selected' : '').'>';
			if($this->diafan->_admin->module == 'photo')
			{
				$html .= $this->diafan->_('Удалить из альбома');
			}
			else
			{
				$html .= $this->diafan->_('Удалить из категории');
			}
			$html .= '</option>';
		}
		...
		$html .= '</select>';
		$html .= $this->diafan->group_action_panel_filter($value).'</div>

			<button class="btn btn_blue btn_small group_actions">'.$this->diafan->_('Применить').'</button>';
		if($show_filter)
		{
			$html .= '
	...
	public function group_action_panel_filter($value)
	{
		...
		if ($this->diafan->is_variable("menu") && $this->diafan->_users->roles('edit'))
		{
			...
			{
				$dop .= '<input type="checkbox" name="menu_cat_ids[]" id="input_menu_cat_ids_'.$row["id"].'" value="'.$row["id"].'" class="group_menu_cat_id"> <label for="input_menu_cat_ids_'.$row["id"].'">'.$row["name"].'</label><br>';
			}
			$dop .= "</div>";
		}
		...
		if ($this->diafan->config('element') && count($this->diafan->categories))
		{
			$dop .= '<div class="action-popup dop_element dop_element_multi dop_element_del'.(! in_array($value, array('element', 'element_multi', 'element_del')) ? ' hide' : '').'">';
			if ($this->diafan->config('element_multiple'))
			{
				$cats = array();
				$count = 0;
			...
			{
				$dop .= '<select name="cat_id">';
				$dop .= $this->diafan->get_options(array(), $this->diafan->categories, array($this->diafan->_route->cat)).'</select>';
			}
			$dop .= '</div>';
		}
