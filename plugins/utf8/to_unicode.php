	...
	for ($i = 0; $i < $len; $i++)
	{
		...
		if ($mState == 0)
		{
			...
			else
			{
				...
				trigger_error('utf::to_unicode: Illegal sequence identifier in UTF-8 at byte '.$i, E_USER_WARNING);
				return false;
			}
		}
		else
		{
			...
			if (0x80 == (0xC0 & $in))
			{
				...
				if (0 == --$mState)
				{
					...
					if (((2 == $mBytes) AND ($mUcs4 < 0x0080)) OR
						...
						($mUcs4 > 0x10FFFF))
					{
						trigger_error('utf::to_unicode: Illegal sequence or codepoint in UTF-8 at byte '.$i, E_USER_WARNING);
						return false;
					}
			...
			else
			{
				...
				trigger_error('utf::to_unicode: Incomplete multi-octet sequence in UTF-8 at byte '.$i, E_USER_WARNING);
				return false;
			}
		}
	}
